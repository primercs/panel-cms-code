...
if($CadUser == "Todos"){
	...
	$SQL = "SELECT id, CadUser, nome, usuario, senha, email, foto, perfil, bloqueado, inativo, data_premio, PrePago, Cota, CotaDias, celular FROM rev WHERE FIND_IN_SET(CadUser,:CadUser)".$PesStatus.$PesPerfil."";
}else{
	
	$SQL = "SELECT id, CadUser, nome, usuario, senha, email, foto, perfil, bloqueado, inativo, data_premio, PrePago, Cota, CotaDias, celular FROM rev WHERE CadUser = :CadUser".$PesStatus.$PesPerfil."";
	}
