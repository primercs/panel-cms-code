...
if($CadUser == "Todos"){
	...
	$SQL = "SELECT id, CadUser, nome, usuario, senha, email, foto, perfil, bloqueado, data_premio, conexao, xml, celular FROM usuario WHERE FIND_IN_SET(CadUser,:CadUser)".$PesStatus.$PesPerfil."";
}else{	
	$SQL = "SELECT id, CadUser, nome, usuario, senha, email, foto, perfil, bloqueado, data_premio, conexao, xml, celular FROM usuario WHERE CadUser = :CadUser".$PesStatus.$PesPerfil."";
	}
