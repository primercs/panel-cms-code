...
if($status == "Ativos"){
	...
	$PesStatus = " AND newt.bloqueado = 'N' AND newt.data_premio >= '".$DataAtual."'";
}
elseif($status == "Bloqueados"){
	$PesStatus = " AND newt.bloqueado = 'S'";
}
elseif($status == "Esgotados"){
	$DataAtual = strtotime(date('Y-m-d'));
	$PesStatus = " AND newt.data_premio < '".$DataAtual."'";
}
else{
	$PesStatus = "";
}
...
if($PerfilVer == 'Todos'){
	...
	$PesPerfil = " AND newt.perfil LIKE '%".$PerfilVer."%'";
}
...
UNION ALL 
SELECT id, CadUser, nome, usuario, senha, email, foto, perfil, bloqueado, data_premio, conexao, xml, celular, grupo, PrePago FROM usuario) as newt WHERE FIND_IN_SET(newt.CadUser,:CadUser)".$PesStatus.$PesPerfil.$PesquisarEntreDatasSQL." OR FIND_IN_SET(newt.CadUser,:CadUser)".$PesStatus.$PesPerfil.$PesquisarEntreDatasSQL2." OR FIND_IN_SET(newt.CadUser,:CadUser)".$PesStatus.$PesPerfil.$PesquisarEntreDatasSQL3."";
...
SELECT id, CadUser, nome, usuario, senha, email, foto, perfil, bloqueado, data_premio, conexao, xml, celular, grupo, PrePago FROM teste 
UNION ALL 
SELECT id, CadUser, nome, usuario, senha, email, foto, perfil, bloqueado, data_premio, conexao, xml, celular, grupo, PrePago FROM usuario) as newt WHERE newt.CadUser = :CadUser".$PesStatus.$PesPerfil.$PesquisarEntreDatasSQL." OR newt.CadUser = :CadUser".$PesStatus.$PesPerfil.$PesquisarEntreDatasSQL2." OR newt.CadUser = :CadUser".$PesStatus.$PesPerfil.$PesquisarEntreDatasSQL3."";
