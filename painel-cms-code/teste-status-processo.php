...
if($CadUser == "Todos"){
	...
	$SQL = "SELECT id, CadUser, nome, usuario, senha, email, foto, perfil, bloqueado, data_premio, conexao, xml, celular FROM teste WHERE FIND_IN_SET(CadUser,:CadUser)".$PesStatus.$PesPerfil."";
}else{
	
	$SQL = "SELECT id, CadUser, nome, usuario, senha, email, foto, perfil, bloqueado, data_premio, conexao, xml, celular FROM teste WHERE CadUser = :CadUser".$PesStatus.$PesPerfil."";
	}
