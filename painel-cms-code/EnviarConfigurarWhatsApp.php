...
$CadUser = InfoUser(2);
	...
	elseif(empty($RedeSocial)){
		echo MensagemAlerta($_TRA['erro'], "WhatsApp é um campo obrigatório!", "danger");
	}
	elseif(is_numeric(LimparCelular($RedeSocial)) == false){
		echo MensagemAlerta($_TRA['erro'], "WhatsApp deve conter apenas números!", "danger");
	}
	else{
	...
	if(empty($SQL)){
		...
		echo MensagemAlerta($_TRA['sucesso'], "WhatsApp configurado com sucesso!", "success", "index.php?p=whatsapp");
	}
